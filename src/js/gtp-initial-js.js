...
(() => {
    ...
    if (slider) {
        ...
        const banners = Array.from(slider.querySelectorAll('.banner'));
        const bannerImages = [
            ...
            'https://images.unsplash.com/photo-1518609878373-06d740f60d8b?auto=format&fit=crop&w=1400&q=70',
        ...
        const computePositions = () => {
            ...
            banners.forEach((banner) => {
                const left = Math.min(banner.offsetLeft, maxScrollLeft);
                if (!nextPositions.length || Math.abs(nextPositions[nextPositions.length - 1] - left) > 1) {
        ...
        const scrollToIndex = (index) => {
            ...
            slider.scrollTo({ left: positions[safeIndex] || 0, behavior: 'smooth' });
        };
        ...
        const buildDots = () => {
            ...
            positions.forEach((_, index) => {
                ...
                btn.className = 'dot';
                btn.setAttribute('aria-label', `Баннер ${index + 1}`);
        ...
        const setActiveDot = () => {
            ...
            positions.forEach((pos, i) => {
                const delta = Math.abs(current - pos);
                if (delta < minDelta) {
                    minDelta = delta;
                    closestIndex = i;
                }
        ...
        banners.forEach((banner, index) => {
            ...
            banner.style.backgroundImage = `url('${bannerImages[index % bannerImages.length]}')`;
        });
        ...
        let timer = setInterval(step, 3000);
